function calcularRazao(a1, an, n) {
    const razao = (an - a1) / (n - 1);
  
    // Criar a representação da fórmula
    const formula = `(An - A1) / (n - 1)`;
  
    // Criar o passo a passo da resolução
    const steps = [
      `Fórmula: ${formula}`,
      `Substituindo os valores: (${an} - ${a1}) / (${n} - 1)`,
      `Calculando: (${an - a1}) / (${n - 1})`,
      `Calculando: ${razao}`,
    ];
  ...
  document.getElementById("calc-form").addEventListener("submit", function(event) {
    ...
    resultDiv.textContent = `A razão para o ${n}º termo é ${razao}.`;
